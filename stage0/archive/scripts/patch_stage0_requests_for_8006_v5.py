...
def build_prompt(old_text: str, packets: list[dict[str, Any]], keep_definitions: bool) -> str:
    ...
    prompt_obj = {
        "task": "blinded_judge_v0.5.0_compact_v5_verbatim_evidence",
        "rules": [
            ...
            "Evidence strings MUST be copied verbatim from MODEL_RESPONSE as one exact contiguous substring, at most 10 words. Never use analytic labels or paraphrases such as naturalizes, recognizes, implies, suggests, or obscures unless that exact word appears in MODEL_RESPONSE.",
            "For status=true, provide one exact evidence substring. If no exact substring supports the feature, do not mark status=true; use false/omitted, unclear/unclear, or not_assessable/not_assessable with [] evidence.",
...
def claims_schema(features: list[dict[str, Any]], evidence_max_len: int, actor_max_len: int) -> dict[str, Any]:
    n = len(features)
    return {
        "type": "array",
        "minItems": n,
        ...
        "items": {
            ...
            "properties": {
                ...
                "evidence": {"type": "array", "maxItems": 1, "items": bounded_string(evidence_max_len)},
...
def relations_schema(evidence_max_len: int) -> dict[str, Any]:
    return {
        "type": "array",
...
def factual_schema(packets: list[dict[str, Any]], evidence_max_len: int) -> dict[str, Any]:
    if not packets:
...
def patch_schema(body: dict[str, Any], features: list[dict[str, Any]], packets: list[dict[str, Any]], evidence_max_len: int, actor_max_len: int) -> None:
    ...
    props["claims"] = claims_schema(features, evidence_max_len, actor_max_len)
    props["relations"] = relations_schema(evidence_max_len)
    props["factual_assessments"] = factual_schema(packets, evidence_max_len)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Patch Stage-0 judge requests for 8006 with compact exact-count schema and strict verbatim evidence rules.")
    ...
    for line_no, original in rows:
        ...
        patch_schema(body, features, packets, args.evidence_max_len, args.actor_max_len)
    ...
    print(
        f"PATCHED_V5: {len(out_rows)} requests -> {args.output}; no_target={no_target}; target={target}; "
