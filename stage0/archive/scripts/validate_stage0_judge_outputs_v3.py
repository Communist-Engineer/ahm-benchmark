...
def validate_one(line_no: int, row: dict[str, Any], req: dict[str, Any] | None, strict_failed_rows: bool, errors: list[str], warnings: list[str]) -> None:
    ...
    for idx, claim in enumerate(claims):
        ...
        if claim.get("status") == "false" and claim.get("disposition") == "instantiated":
            fail(errors, line_no, custom_id, f"claim {idx} has forbidden false/instantiated pair")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Validate Stage-0 judge output structural completeness after v3 request patching.")
    ap.add_argument("--outputs", required=True, type=Path)
    ap.add_argument("--requests", type=Path, help="Patched request JSONL used for this run. Enables factual-target cross-checks.")
    ...
    for line_no, row in output_rows:
        ...
        validate_one(line_no, row, req, args.strict_failed_rows, errors, warnings)
