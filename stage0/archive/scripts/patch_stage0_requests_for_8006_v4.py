...
def build_prompt(old_text: str, packets: list[dict[str, Any]], keep_definitions: bool) -> str:
    ...
    prompt_obj = {
        "task": "blinded_judge_v0.5.0_compact",
        "rules": [
            ...
            "Evidence strings must be exact substrings of MODEL_RESPONSE, at most 12 words. Use [] for omitted, not_assessable, or when no exact short span is needed.",
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Patch Stage-0 judge requests for 8006 with a shorter prompt and compact exact-count schema.")
    ...
    print(
        f"PATCHED_V4: {len(out_rows)} requests -> {args.output}; no_target={no_target}; target={target}; "
